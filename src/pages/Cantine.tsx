...
export default function Cantine() {
  ...
  const [entries, setEntries] = useState<CantineEntry[]>([]);
    ...
    try {
      const [entryRes, personRes] = await Promise.all([
        supabase
          .from('cantine')
          .select(
            'id, tenant_id, person_id, meal_type, meal_date, notes, created_at, ' +
      ...
      setEntries((entryRes.data || []) as unknown as CantineEntry[]);
  ...
  function getPersonName(entry: CantineEntry): string {
    if (entry.person) {
  ...
  const filtered = entries.filter((e) => {
    if (filterDate && e.meal_date !== filterDate) return false;
    if (filterMeal !== 'all' && e.meal_type !== filterMeal) return false;
    if (search) {
  ...
  const todayEntries = entries.filter((e) => e.meal_date === new Date().toISOString().split('T')[0]);
  const todayTotal = todayEntries.length;
  const todayBreakfast = todayEntries.filter((e) => e.meal_type === 'breakfast').length;
  const todayLunch = todayEntries.filter((e) => e.meal_type === 'lunch').length;
  const todayDinner = todayEntries.filter((e) => e.meal_type === 'dinner').length;

  const mealBadgeColor: Record<MealType, string> = {
  ...
  async function handleAdd() {
    ...
    setSubmitting(true);
    try {
      const { error } = await supabase.from('cantine').insert({
        tenant_id: tenant.id,
        person_id: addForm.person_id,
        meal_type: addForm.meal_type,
        meal_date: addForm.meal_date,
        notes: addForm.notes,
      });
      ...
      console.error('Failed to add cantine entry:', err);
    }
    setSubmitting(false);
  }
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      console.error('Failed to delete cantine entry:', err);
    }
  }
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-900">{t('cantines')}</h1>
        <p className="text-slate-500 mt-1">{t('stats')}</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
          <div className="text-sm text-slate-500">{t('breakfast')}</div>
          <div className="text-2xl font-bold text-amber-700">{todayBreakfast}</div>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
          <div className="text-sm text-slate-500">{t('lunch')}</div>
          <div className="text-2xl font-bold text-emerald-700">{todayLunch}</div>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
          <div className="text-sm text-slate-500">{t('dinner')}</div>
          <div className="text-2xl font-bold text-indigo-700">{todayDinner}</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="p-4 border-b border-slate-100 flex flex-col gap-3">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <select
              value={filterMeal}
              onChange={(e) => setFilterMeal(e.target.value as MealFilter)}
              className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 appearance-none"
            >
          ...
          <div className="p-4 border-b border-slate-100 bg-slate-50">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
              ...
              <select
                ...
                onChange={(e) => setAddForm({ ...addForm, meal_type: e.target.value as MealType })}
                className="px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
              >
                <option value="breakfast">{t('breakfast')}</option>
                <option value="lunch">{t('lunch')}</option>
              ...
              <input
                type="date"
                value={addForm.meal_date}
                onChange={(e) => setAddForm({ ...addForm, meal_date: e.target.value })}
                className="px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
              />
                ...
                {filtered.map((e) => (
                  <tr key={e.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${mealBadgeColor[e.meal_type]}`}
                      >
                        {t(e.meal_type)}
                      </span>
                    </td>
